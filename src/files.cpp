...
void changeDir(std::string newPath) {
  ...
  // if (fs::current_path(newPath)) { /* Doesn't return error */ /* Doesn't
  // return error */
  if (cd(newPath.c_str())) {
...
std::string getPerms(std::string entry) { /* source:
...
std::string getSize(std::string entry) {
  return (fs::is_directory(fs::status(std::string))
              ? "DIR"
              : toHumanReadableSize(fs::file_size(entry)));
}

void remove(std::string entry) {
  /* TODO: move question to tui */
  std::cout << "Are you sure you want to delete the file/folder and all its "
               "subfolders?";
  char chr;
  std::cin >> chr;
  if (chr != 'y' && chr != 'Y')
...
void copy(std::string from, std::string to) { fs::copy(from, to); }
...
void move(std::string from, std::string to) { fs::rename(from, to); }
...
void TEST_printVec(auto v) {
  for (long unsigned i = 0; i < v.size(); i++) {
    std::cout << getPerms(v.at(i)) << " " << v.at(i).filename().string() << " "
              << getSize(v.at(i)) << "\n"; // get filename
  }
}
